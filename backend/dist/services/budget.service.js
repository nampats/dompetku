...
export const budgetService = {
    ...
    async upsert(userId, data) {
        ...
        const [existing] = await db
            .select()
            .from(budgets)
            .where(and(eq(budgets.userId, userId), eq(budgets.categoryId, data.categoryId), eq(budgets.month, data.month), eq(budgets.year, data.year)));
        if (existing) {
    ...
    async remove(userId, id) {
        ...
        if (!budget)
            throw createError('Anggaran tidak ditemukan.', 404, 'NOT_FOUND');
