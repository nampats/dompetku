...
export const debtService = {
  async list(userId: string, type?: 'utang' | 'piutang', status?: string) {
    const conditions = [eq(debts.userId, userId)];
    if (type) conditions.push(eq(debts.type, type));
    if (status) conditions.push(eq(debts.status, status as any));
  ...
  async getById(userId: string, id: string) {
    ...
    if (!result) throw createError('Catatan hutang/piutang tidak ditemukan.', 404, 'NOT_FOUND');
    return result;
  },
  ...
  async addPayment(
    userId: string,
    debtId: string,
    data: { amount: string; paidAt: string; note?: string }
  ) {
